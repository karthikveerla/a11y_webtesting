...
export default function ImageTable() {
  return (
    <main className="mx-auto max-w-3xl px-4 py-8">
      ...
      <SectionTitle id="simple">1. Semantic Simple</SectionTitle>
      <Paragraph>
        A classic data table with one header row and row headers. Ideal for checks like header
        presence, scope correctness, and consistent cell counts.
      </Paragraph>
      <SemanticSimpleTable />

      <SectionTitle id="complex">2. Semantic Complex</SectionTitle>
      <Paragraph>
        Demonstrates grouped headers and multi-level column labeling. Good for IDREF association or
        multi-header announcement validations.
      </Paragraph>
      <SemanticComplexTable />

      <SectionTitle id="layout">3. Layout Table (Presentation)</SectionTitle>
      <Paragraph>
        Uses a table for visual layout only, marked with <code>role="presentation"</code>. Later we can
        intentionally misuse roles or add headers to see how checks respond.
      </Paragraph>
      <LayoutTable />

      <SectionTitle id="grid">4. Grid (Div-based)</SectionTitle>
      <Paragraph>
        An ARIA grid-like table built with <code>div</code>s. Currently includes appropriate roles. Easy to
        break intentionally by removing roles, captions, or changing structure.
      </Paragraph>
      <GridTable />

      <Paragraph>
        Need more fixtures (e.g., image-only table, offscreen table, caption variants)? Say the word
        and we’ll add them as separate sections without disturbing these baselines.
      </Paragraph>


      <Paragraph>
        This table is provided only as an image and contains no semantic HTML structure. It should FAIL.
      </Paragraph>

      <Table_Fail_ImageOnly />
    </main>
  );
}
